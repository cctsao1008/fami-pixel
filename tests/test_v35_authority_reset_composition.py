...
def test_reset_plan_is_staged_but_not_double_applied_before_wrapper_extraction():
    v35 = _load_v35()
    source = inspect.getsource(v35.authority_main)

    # Historical wrappers still own the active reset calls.  Until those calls
    # are removed in the next migration slice, invoking the stable plan here
    # would double-reset state and obscure exact behavioral equivalence.
    assert "_AUTHORITY_RUN_RESET_PLAN.reset_run_state()" not in source
