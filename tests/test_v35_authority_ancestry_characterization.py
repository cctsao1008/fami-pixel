...
def test_current_authority_runtime_delegation_chain_is_exact():
    ...
    # Current V35 owns the former V34/V33/V32 reset/log prefix and now enters the
    # still-historical authority ancestry at V30.
    expected_current_delegates = (
        (v35.authority_main, "return _V30.authority_main(args)"),
    ...
    # Historical V34/V33/V32 examples remain runnable with their original
    # standalone delegation and reset ownership, even though current V35 bypasses
    # them.
...
def test_authority_wrappers_preserve_their_current_stateful_responsibilities():
    ...
    # owns the first three reset calls at the stable root.
    ...
    # occurrence moved to V35 while the V29 occurrence still executes below V30.
...
def test_authority_extraction_boundary_is_runtime_orchestration_not_policy_rewrite():
    ...
    # The runtime ancestry has shortened only by bypassing reset/log wrappers.
    assert "_COLLECT_PROGRESS_CONTROL.decide(" in inspect.getsource(v35._best_collect_or_progress)
    assert "return _V30.authority_main(args)" in inspect.getsource(v35.authority_main)
