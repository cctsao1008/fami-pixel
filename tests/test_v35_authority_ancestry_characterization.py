...
def test_current_authority_runtime_delegation_chain_is_exact():
    ...
    # Current V35 owns the former V34/V33 reset-only prefix and now enters the
    # still-historical authority ancestry at V32.
    expected_current_delegates = (
        (v35.authority_main, "return _V32.authority_main(args)"),
    ...
    # Historical V34/V33 examples remain runnable with their original standalone
    # delegation and reset ownership, even though current V35 bypasses them.
...
def test_authority_wrappers_preserve_their_current_stateful_responsibilities():
    ...
    assert '_AUTHORITY_RUN_RESET_PLAN.reset_through("v33-deadline-cache")' in v35_source
    assert "return _V32.authority_main(args)" in v35_source
    ...
    # Standalone historical runners keep provenance behavior; only current V35
    # has transferred these two outer resets to the stable reset plan.
...
def test_authority_extraction_boundary_is_runtime_orchestration_not_policy_rewrite():
    ...
    # The runtime ancestry has shortened only by bypassing two reset/log wrappers.
    assert "_COLLECT_PROGRESS_CONTROL.decide(" in inspect.getsource(v35._best_collect_or_progress)
    assert "return _V32.authority_main(args)" in inspect.getsource(v35.authority_main)
